...
myfile = open('hw4_problem2_testing_data.pickle', 'rb')
...
for i in range(n):
    for j in range(n):
        K[i][j] = sigma_f**2 * np.exp(-1 * (X_train[i] - X_train[j])**2 / (2*ls**2)) + sigma**2 * (i == j)

for idx in range(X_test.shape[0]):
    for i in range(n):
        K[i][n] = sigma_f**2 * np.exp(-1 * (X_train[i] - X_test[idx])**2 /(2*ls**2)) + sigma**2 * (i == n)
        K[n][i] = sigma_f**2 * np.exp(-1 * (X_train[i] - X_test[idx])**2 /(2*ls**2)) + sigma**2 * (i == n)
    K[n][n] = sigma_f**2 * np.exp(-1 * (X_test[idx] - X_test[idx])**2 / (2*ls**2)) + sigma**2
...
fig.tight_layout()
plt.savefig(f"./figures/p2_sig={sigma}_sigf={sigma_f}_ls={ls}.png", dpi=300)
